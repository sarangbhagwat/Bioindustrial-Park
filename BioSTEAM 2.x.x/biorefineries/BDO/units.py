...
from BDO.utils import baseline_feedflow, compute_extra_chemical, adjust_recycle
...
class SaccharificationAndCoFermentation(Unit):    
    ...
    def _run(self):
        ...
        # !!! Temporary alteration for 2,3-BDO production
...
class CoFermentation(Unit):    
    ...
    def __init__(self, ID='', ins=None, outs=(), T=30+273.15):
        ...
        self.cofermentation_rxns = ParallelRxn([
        #      Reaction definition            Reactant    Conversion
        Rxn('Glucose -> BDO + 2CO2',        'Glucose',   .90), 
        Rxn('Glucose -> Acetoin + 2CO2',               'Glucose',   0.065),
        Rxn('Glucose -> 6 FermMicrobe',       'Glucose',   0.03),
        Rxn('Xylose -> BDO + 2CO2',       'Xylose',    0.72),
        Rxn('Xylose -> Acetoin + 2CO2',       'Xylose',    0.052),
...
class SeedTrain(Unit):
    ...
    def __init__(self, ID='', ins=None, outs=(), T=50+273.15, ferm_ratio=0.9):
        ...
        self.cofermentation_rxns = ParallelRxn([
        #      Reaction definition            Reactant    Conversion
        Rxn('Glucose -> BDO + 2CO2',        'Glucose',   .80), # .9099
        Rxn('Glucose -> Acetoin + 2CO2',               'Glucose',   0.065),
        Rxn('Glucose -> 6 FermMicrobe',       'Glucose',   0.03),
        Rxn('Xylose -> BDO + CO2',       'Xylose',    0.0),
        Rxn('Xylose -> Acetoin + CO2',       'Xylose',    0.0),
        Rxn('Xylose -> 5 FermMicrobe',        'Xylose',    0.0),
        ])
...
@cost(basis='Solids flow rate', ID='Pressure filter', units='kg/hr',
      cost=3294700, S=28630, CE=550.8, n=0.8, BM=1.7)
...
class BDOStorageTank(StorageTank):
    def _cost(self):
...
class DehydrationReactor(Reactor):
    ...
    dehydration_rxns = ParallelRxn([
            #   Reaction definition                                       Reactant   Conversion
            Rxn('BDO -> MEK + H2O',         'BDO',   0.52),
            Rxn('BDO -> IBA + H2O',         'BDO',   0.26)
                ])
    ...
    def _run(self):
        ...
        effluent = feed.copy_like()
        
        rxns = self.dehydration_rxns
        rxns(effluent.mol)
